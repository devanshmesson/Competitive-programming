...
int unbounded(int weight[],int val[],int capacity,int n,int** dp)
{
  //if already calculated,save time!
  ...
  */
  ...
  {
    return dp[capacity][n]=max(val[n]+unbounded(weight,val,capacity-weight[n],n,dp) //include the item multiple times (stayed on that index)
                                     ,unbounded(weight,val,capacity,n-1,dp)); //skip this item,so move to next item(index)
  }
  else//Case:2
  {
    return dp[capacity][n]=unbounded(weight,val,capacity,n-1,dp); //skip this item,so move to next item(index)
  }
}
int main()
{
  ...
  */
  ...
  //make all values of dp as -1
  for(int i=0;i<=capacity;i++)for(int j=0;j<=n;j++)dp[i][j]=-1;
  
  cout<<unbounded(weight,val,capacity,n,dp)<<endl; 
  return 0;
}
